...
#pragma warning disable CS8604 // Возможно, аргумент-ссылка, допускающий значение NULL.
    ...
    public class OSPClient : IDisposable
    {
        ...
        public Task Start()
        {
            ...
            byte[] ecnryptedKey = rsa_provider.Encrypt(aes.Key, RSAEncryptionPadding.Pkcs1);
            List<byte> sendData = new List<byte>();
            sendData.AddRange(ecnryptedKey);
            sendData.AddRange(terminator);
            sendData.AddRange(aes.IV);
            sendData.Add(0x1E);
            sendData.Add(0x1F);
            sendData.Add(0x1E);
            sendData.Add(0x1F);
         ...
         //   header;
            ...
            while (true)
            {
                try
                {
                    ...
                    byte[] decoded = Decrypt(HedBuffer);
                    HeaderMessage header = MakeHeaderFromResponse(decoded, IPEndPoint.Parse($"{_ip}:{_port}"));




                    if (!_allRequests.ContainsKey(header.UniID)) NewMessageSent(header);



                    if (header.DataLength == 0)
                    {
                        _ = Task.Run(() => _allRequests[header.UniID].SetResult(new OSPResponse() { Data = null, Header = header, StatusCode = header.MessageStatus, OnlyStatusCode = true }));
                        data.Clear();
                    }
                    else
                    {
                        ...
                        {
                            int bytesCount = await stream.ReadAsync(dataBuffer, 0, dataBuffer.Length);
                            ...
                            {
                                if (!_allRequests.ContainsKey(header.UniID))
                                {
                            ...
                            if (progress == 1)
                            {
                                ...
                                {
                                    byte[] buffer = Decrypt(data.ToArray());
                                    _ = Task.Run(() => _allRequests[header.UniID].SetResult(new OSPResponse() { Data = buffer, Header = header, StatusCode = header.MessageStatus, OnlyStatusCode = false }));
                                    data.Clear();
        ...
        public OSPClient(string ip, int port)
        {
            ...
            aes.KeySize = Settings.AES_Size;

            _ip = ip;
            _port = port;
        }

        Dictionary<uint, TaskCompletionSource<OSPResponse>> _allRequests = new Dictionary<uint, TaskCompletionSource<OSPResponse>>();
        ...
        byte[] MakeRequest(string description, OSPStatusCode code, byte[] body, uint ID)
        {
            ...
            byte[] headerbytes = Encrypt(Encoding.UTF8.GetBytes(header));
                    ...
                    while ((bytesRead = stream.ReadByte()) != -1)
                    {
                        ...
                        if (data.Count >= 4)
                        {
                            ...
                            if (data[data.Count - 1] == 0x4D && data[data.Count - 2] == 0x3C && data[data.Count - 3] == 0x2B && data[data.Count - 4] == 0x1A)
                            {
                                ...
                                aes.Key = rsa_provider.Decrypt(data.ToArray(), RSAEncryptionPadding.Pkcs1);
